...
def ari(real_partition, method, kmin, kmax, file):
	...
	try:
		rand_file = open('results/' + file + '/' + method + '-' + file + '-ari.csv', 'w')
		rand_file.write('Número de clusters \t ARI\n')
		for i in range(kmin, kmax+1):
			print('results/' + file + '/' + method +  '-' + file + '-k=' + str(i) + '.clu')
			data = pd.read_csv('results/' + file + '/' + method +  '-' + file + '-k=' + str(i) + '.clu', delimiter = '\t')
...
def main():
	...
	if op == 1:
		...
		#ari(real_partition, 'avg', 2, 5, 'c2ds1-2sp')
		...
		#ari(real_partition, 'avg', 2, 5, 'c2ds3-2g')
