...
def main() :
	...
	for i in range(qtd-1):
		...
		if (k >= k_Min and k <= k_Max):
			...
			save[[save.columns[0], 'cluster']].to_csv("results/" + arq + "/single-" + arq + "-k=" + str(k) + ".clu", sep='\t', encoding='utf-8', index = False, header = False)
			...
			plt.savefig("results/" + arq + "/plots/single-" + arq + "-k=" + str(k) + ".png")
